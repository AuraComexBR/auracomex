...
// ============================================================================
// TrackingV3 — reorganiza a área expandida do processo (fork de TrackingV2).
// Página PARALELA à Tracking.tsx/TrackingV2.tsx (não usada em produção
// ainda), pra validação antes de substituir a atual. Mesma lógica de dados
// (auth por PIN, queries, docs, diário) — a diferença é só a organização
// visual do card expandido de cada processo:
//
//   - Dados do processo (Shipper/Armador/Navio/rota/transit/free time,
//     Invoice/BL/Containers e Resumo da Carga) viram tabelas tipo planilha
//     (cabeçalho de colunas + linha(s) de valor) em vez de cards soltos.
//   - Diário do Processo sobe pro topo do card expandido, com destaque
//     visual (borda/fundo diferenciados) — é o que o cliente mais quer ver.
//   - Documentos vira uma seção colapsável (fechada por padrão) no fim.
// ============================================================================
...
export default function TrackingV3() {
...
/** Tabela compacta tipo planilha — cabeçalho de colunas + uma ou mais linhas
 *  de valor. Usada nos blocos de dados do processo expandido (v3), no lugar
 *  dos cards soltos de campo único que a v2 usava. */
function SpreadsheetTable({ columns, rows }: { columns: { key: string; label: string }[]; rows: Record<string, React.ReactNode>[] }) {
      ...
      <table className="w-full text-xs border-collapse">
        <thead>
          <tr className="bg-muted/50">
            {columns.map((c) => (
              <th key={c.key} className="px-3 py-1.5 text-left font-medium text-muted-foreground uppercase tracking-wide text-[10px] whitespace-nowrap border-b border-border">
                {c.label}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className="even:bg-muted/20">
              {columns.map((c) => (
                <td key={c.key} className="px-3 py-1.5 font-semibold whitespace-nowrap border-b border-border last:border-b-0">
                  {row[c.key] ?? '—'}
                </td>
              ))}
            </tr>
      ...
      {expanded && (
        <TableRow className="hover:bg-transparent">
          <TableCell colSpan={colSpan} className="p-0">
            ...
            {/* Lista de documentos — colapsada por padrão (v3) */}
