  ...
  try {
    ...
    if (role === "superadmin") {
      if (body?.scope === "all") {
        ...
        targetCompanyId = String(body.companyId);
      } else {
        ...
        const { data: prof } = await admin
          ...
          .eq("user_id", caller.id)
          .single();
        targetCompanyId = prof?.company_id ?? null;
      }
    } else {
      // admin / diretor: forced to own company
      const { data: prof } = await admin
        .from("profiles")
        .select("company_id")
        .eq("user_id", caller.id)
    ...
    if (scope === "all") {
      filenameScope = "all-companies";
      const { data: companies } = await admin
        .from("companies")
        .select("id, name")
        .order("name");
      sql = header(`ALL companies (${companies?.length ?? 0})`);
      for (const c of companies ?? []) {
        sql += await dumpCompany(admin, c.id, c.name);
      }
      sql += footer;
    } else {
    ...
    try {
      await admin.from("activity_log").insert({
        company_id: scope === "all" ? null : targetCompanyId,
        user_id: caller.id,
